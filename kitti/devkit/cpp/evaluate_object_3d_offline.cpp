...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
  ...
  if (true){
    ...
    cout << "pr[thresholds.size()-1].tp = " << pr[thresholds.size()-1].tp << endl;
    cout << "pr[thresholds.size()-1].fp = " << pr[thresholds.size()-1].fp << endl;
    cout << "Total number of labels = " << pr[0].tp + pr[0].fn << endl;
    cout << "Total number of detections = " << pr[thresholds.size()-1].tp + pr[thresholds.size()-1].fp << endl;
  }
...
bool eval(string gt_dir, string result_dir, string output_dir, Mail* mail){
  ...
  for (int32_t i=0; i<indices.size(); i++) {
    ...
    // check for errors
    if (!gt_success) {
      mail->msg("ERROR: Couldn't read: %s of ground truth.", gt_dir + file_name);
      return false;
    }
    if (!det_success) {
      mail->msg("ERROR: Couldn't read: %s of detection.", result_dir + file_name);
      return false;
    }
  }
  mail->msg("  done.");
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      fp_det = fopen((output_dir + "stats_" + CLASS_NAMES[c] + "_detection.txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((output_dir + "stats_" + CLASS_NAMES[c] + "_orientation.txt").c_str(),"w");
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      fp_det = fopen((output_dir + "stats_" + CLASS_NAMES[c] + "_detection_ground.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_3d[c]) {
      fp_det = fopen((output_dir + "stats_" + CLASS_NAMES[c] + "_detection_3d.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
...
int32_t main (int32_t argc,char *argv[]) {
  if (argc!=4) {
    cout << "Usage: ./eval_detection_3d_offline gt_dir result_dir output_dir" << endl;
    return 1;
  }
  ...
  // run evaluation
  if (eval(gt_dir, result_dir, output_dir, mail)) {
    mail->msg("Your evaluation results are available at:");
    mail->msg(output_dir.c_str());
  } else {
    system(("rm -r " + output_dir + "plot").c_str());
